    ...
    const SupportParameters &support_params)
{
    ...
    if (support_params.tree_branch_infill_density > 0)
    ...
    for (ExPolygon expoly : closing_ex(polygons, float(SCALED_EPSILON), float(SCALED_EPSILON + 0.5 * flow.scaled_width()))) {
        size_t num_walls = support_params.tree_branch_walls;
        if (support_params.tree_branch_diameter_double_wall_area_scaled > 0) {
            ...
            if (area > support_params.tree_branch_diameter_double_wall_area_scaled)
                num_walls = std::max<size_t>(num_walls, 2);
        }

        for (size_t i = 0; i < num_walls && !expoly.contour.points.empty(); ++i) {
            extrusion_entities_append_paths(dst, draw_perimeters(expoly, clip_length, support_params.prefer_clockwise_movements),
                { ExtrusionRole::SupportMaterial, flow }, false);
            if (i + 1 < num_walls) {
                ExPolygons inner = offset2_ex({ expoly }, -flow.scaled_spacing(), 0.5 * flow.scaled_spacing());
                if (inner.empty()) {
                    expoly.contour.points.clear();
                } else {
                    expoly = inner.front();
                }
            }
        ...
        if (support_params.tree_branch_infill_density > 0 && !expoly.contour.points.empty()) {
            if (!filler)
                filler = std::unique_ptr<Fill>(Fill::new_from_type(support_params.base_fill_pattern));
            filler->set_bounding_box(expoly.bounding_box());
            ...
            ExPolygons exps;
            exps.emplace_back(expoly);
            fill_expolygons_generate_paths(dst, std::move(exps), filler.get(), float(support_params.tree_branch_infill_density),
